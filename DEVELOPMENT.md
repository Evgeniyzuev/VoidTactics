...
- `SensorService` — единственный источник уровней контакта для рендера, AI, команд, tooltip и событий.
- `FlightModel` — единственный источник физики движения для игрока, NPC и автопилота: инерционный разгон, лимит поворота, торможение, режимы `impulse`/`warp`. Характеристики берутся из `HULLS` (`maxSpeed`, `acceleration`, `turnRate`), а не задаются в `Fleet`.
...
- `FlightProfile`, `FlightState`, `FlightIntent` — характеристики формации из живых корпусов, состояние (heading/throttle/mode) и намерение автопилота или ручного трима;
...
- инерцию: медленный разгон, лимит поворота, ненулевой тормозной путь, отсутствие мгновенной остановки;
- независимость результата манёвра от шага времени (`dt = 1/60`, `1/30`, `0.1`), пропорциональность ускорения и потолка скорости параметру `performance`;
- прибытие автопилота к точке и удержание дистанции в бою вместо тарана;
- отсутствие NaN и детерминированность длинной серии шагов полёта;
